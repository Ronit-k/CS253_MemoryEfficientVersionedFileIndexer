...
- [`230875_ronit.cpp`](230875_ronit.cpp) — C++ source code
- [`230875_ronit.md`](230875_ronit.md) — README file
- [`230875_ronit.pdf`](230875_ronit.pdf) — Assignment report
- [`230875_ronit.jpg`](230875_ronit.jpg) — Execution screenshot
...
### 1. Clone the submission branch and enter the repository

```bash
git clone -b submission https://github.com/Ronit-k/CS253_MemoryEfficientVersionedFileIndexer.git
...
```bash
g++ -std=c++17 -O2 -o analyzer 230875_ronit.cpp
```
...
```bash
./analyzer --file test_logs.txt --version v1 --buffer 256 --query word --word DEVOPs
```
```
Indexing v1 [########################################] 100%
Version: v1
...
Enter buffer size in KB (256–1024): 256
...
An optional progress bar is displayed during indexing if `progressbar.h` is present alongside `230875_ronit.cpp`. It is auto-detected at compile time via `__has_include` — if the header is absent, the program compiles and runs identically without it. The progress bar writes to `stderr` so it does not interfere with query output on `stdout`.
...
```bash
wget https://raw.githubusercontent.com/Ronit-k/CS253_MemoryEfficientVersionedFileIndexer/submission/progressbar.h
```
